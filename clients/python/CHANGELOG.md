...
- `ingest_files(..., allow_oversized=False)` — a client-side guard that rejects
  files larger than 8 MiB with `ValidationError(code="EFILE_TOO_LARGE_FOR_CPYTHON")`
  before anything is sent. The `python3` runtime (CPython WASM) reads files
  through an 8 MiB IPC bridge, so `open()` fails on larger files. Pass
  `allow_oversized=True` to ingest anyway (the bytes stay usable from bash and
  `js-exec`; only `python3 open()` can't read them), or split the file into
  <8 MiB chunks.
