...
- **Rule (outside a script scope):** Reads hit cache first. Writes go to DB first, then update caches. Cache is ephemeral — DB is the source of truth.
- **Rule (inside a script scope, `SCRIPT_TX_BUFFERED=true`, the default):** the caches may LEAD the database, and only until the scope ends (#166). A mutation updates pathCache/contentCache and appends to an in-memory journal; `endScriptScope` replays the journal in one short advisory-locked transaction and commits. File *bytes* are exempt — `commitBlob` still writes them eagerly on its own connection before the metadata is buffered, and the GC grace window (`BLOB_GC_MIN_AGE_MS`) covers a blob that is committed but not yet referenced.
  - The lead is closed on every exit: a failed flush reloads committed state and clears the dirty flag, and if that reload also fails the cache is marked poisoned so no version/snapshot is published. `reload()` stays refused while a scope is open (F4).
  - Inode ids the script has not yet created are negative placeholders, rewritten to real ids after COMMIT. A placeholder that survives the flush is cache/DB divergence and fails the request — it must never reach the Redis path snapshot.
  - DB-surfaced errors move from the failing command to the flush: bash sees the command succeed and the *request* fails afterwards. This only fires when pathCache and the DB have diverged.
  - Per-script all-or-nothing is preserved. At the buffer cap the script fails with `ESCRIPTBUFFER` and nothing is applied; the buffer is never flushed early.
...
| `REDIS_BLOB_SET_MAX_IN_FLIGHT` | No (default: 32) | Max concurrent `RedisBlobCache.set` backfills, counted per data CONNECTION and therefore shared by every tenant on it (the cap belongs to the socket that backs up, not to the per-tenant cache in front of it). The dialect calls `set` fire-and-forget, so over this cap the write is DROPPED (logged as `redis_blob_set_dropped`), never queued — the cache is fail-open, so the cost is one later Postgres read. |
| `REDIS_BLOB_SET_MAX_IN_FLIGHT_BYTES` | No (default: 33554432) | Max total bytes of concurrent `RedisBlobCache.set` backfills (32 MB), likewise per data connection and shared across tenants. Same drop-not-queue rule. |
...
| `MAX_FILE_WRITE_BYTES` | No (default: contentCache cap, 50 MiB) | Largest single file body any write surface accepts (PUT, PATCH, MCP `file_write`, and each entry of `POST /writeFiles` or of the `files` map on `POST /v1/sandboxes`). Raising it past the contentCache cap costs ~4x the memory per file for the whole `SESSION_IDLE_MS`. |
| `MAX_BULK_WRITE_BYTES` | No (default: `MAX_FILE_WRITE_BYTES`) | Largest total decoded byte count one batch write may carry — `POST /writeFiles` and the `files` map on `POST /v1/sandboxes` both. Defaults to the per-file cap so a batch is not a wider door than the single write it batches (#168 — it used to default to 128 MiB against a 50 MiB per-file cap, and the create route kept that 128 MiB default a step longer). The request body is separately capped at twice this, counted off the stream before the JSON parse, to leave headroom for JSON string escaping. |
| `MAX_EXEC_FILE_BYTES` | No (default: 8 MiB) | Largest file a sandbox `exec` script may read whole, or produce with one write. Bash text utilities rebuild strings synchronously on the main thread — `sed s///g` blocks the event loop 706 ms at 8 MiB and 2238 ms at 16 MiB, and past the 2 s Redis `commandTimeout` that stall times out *other* tenants' in-flight commands. Trips as `EFBIG` → 413, never retryable. Applies only to calls made from inside `bash.exec`; the HTTP/MCP file routes keep their own caps, so a file above this can still be fetched with `GET .../files/{path}`. **It holds the 2 s wall PER FILE, not per script**: `cat a b | wc -c` runs two in-cap checks over 16 MiB of pipeline work, and `sed 's/x/xxxxxxxx/g' 8mib.txt` expands one in-cap input into ~64 MiB unchecked. The boundary that would catch those is inside just-bash, which we do not modify — see #198 for the structural fix. Stopgap until `bash.exec` moves off the main thread (#168). |
| `MAX_BULK_WRITE_FILES` | No (default: 1000) | Max number of entries in one batch write (`POST /writeFiles`, or the `files` map on `POST /v1/sandboxes`). |
...
| `SCRIPT_TX_BUFFERED` | No (default: `true`) | Buffer a script's metadata mutations in memory and flush them in one short transaction at scope end, instead of holding one transaction open across the whole of `bash.exec` (#166). The legacy shape pins a pooled server connection for as long as the user's script runs — `sleep`, a Python step, a `git clone` — which wedges a transaction pooler at roughly `default_pool_size` concurrent writers. Set to `false` to restore it without a code change; both shapes stay wired. |
| `SCRIPT_TX_BUFFER_MAX_OPS` | No (default: 50000) | Max buffered mutations in one script scope. Past it the script fails with `ESCRIPTBUFFER` (413) and **nothing is applied** — auto-flushing would split one commit into two and break the per-script all-or-nothing guarantee `ELOCKLOST`'s "not committed" claim rests on. 5x just-bash's own `maxCommandCount` of 10,000, so no single exec can reach it: the heaviest measured script buffered 3,317 ops. |
| `SCRIPT_TX_BUFFER_MAX_BYTES` | No (default: 33554432) | Max estimated bytes of one script's buffered mutations (32 MiB). The buffer holds metadata only — paths, names, shas, ids, and the subtree plans `cp -r`/`rm -r` capture — never file content. |
