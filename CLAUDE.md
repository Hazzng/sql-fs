...
- **Rule (inside a script scope, `SCRIPT_TX_BUFFERED=true`, the default):** the caches may LEAD the database, and only until the scope ends (#166). A mutation updates pathCache/contentCache and appends to an in-memory journal; `endScriptScope` replays the journal in one short advisory-locked transaction and commits. File *bytes* are exempt — `commitBlob` still writes them eagerly on its own connection before the metadata is buffered, and the GC grace window (`BLOB_GC_MIN_AGE_MS`) covers a blob that is committed but not yet referenced.
  ...
  - Per-script all-or-nothing is preserved. At the buffer cap the script fails with `ENOBUFS` and nothing is applied; the buffer is never flushed early.
...
| `SCRIPT_TX_BUFFER_MAX_OPS` | No (default: 50000) | Max buffered mutations in one script scope. Past it the script fails with `ENOBUFS` (413) and **nothing is applied** — auto-flushing would split one commit into two and break the per-script all-or-nothing guarantee `ELOCKLOST`'s "not committed" claim rests on. 5x just-bash's own `maxCommandCount` of 10,000, so no single exec can reach it: the heaviest measured script buffered 3,317 ops. |
