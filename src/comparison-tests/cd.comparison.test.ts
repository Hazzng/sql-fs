...
import { cleanupTestDir, createTestDir, runRealBash, setupFiles } from "./fixture-runner.js";
...
describe("cd command - Real Bash Comparison", () => {
	...
	describe("basic cd", () => {
		...
		it("should change to parent directory with ..", async () => {
			...
			const realResult = await runRealBash("cd parent/child && cd .. && pwd", testDir);
		...
		it("should handle multiple .. in path", async () => {
			...
			const realResult = await runRealBash("cd a/b/c && cd ../.. && pwd", testDir);
	...
	describe("cd -", () => {
		it("should return to previous directory", async () => {
			...
			const realResult = await runRealBash("cd dir1 && cd ../dir2 && cd - && pwd", testDir);
	...
	describe("cd errors", () => {
		it("should error on non-existent directory", async () => {
			...
			const realResult = await runRealBash("cd nonexistent 2>&1; echo $?", testDir);
		...
		it("should error when cd to file", async () => {
			...
			const realResult = await runRealBash("cd file.txt 2>&1; echo $?", testDir);
