...
import { fs, path, cleanupTestDir, createTestDir, runRealBash, setupFiles } from "./fixture-runner.js";
...
describe("tee command - Real Bash Comparison", () => {
	...
	describe("basic usage", () => {
		...
		it("should write to file and stdout", async () => {
			...
			const realResult = await runRealBash("echo hello | tee output.txt", testDir);
		...
		it("should write to multiple files", async () => {
			...
			const realResult = await runRealBash("echo hello | tee file1.txt file2.txt", testDir);

			expect(envResult.stdout).toBe(realResult.stdout);

			// Compare file contents
			const envContent1 = await env.readFile("file1.txt");
			const realContent1 = await fs.readFile(path.join(testDir, "file1.txt"), "utf-8");
			expect(envContent1).toBe(realContent1);

			const envContent2 = await env.readFile("file2.txt");
			const realContent2 = await fs.readFile(path.join(testDir, "file2.txt"), "utf-8");
	...
	describe("append mode", () => {
		it("should append with -a flag", async () => {
			...
			const realContent = await fs.readFile(path.join(testDir, "existing.txt"), "utf-8");
			expect(envContent).toBe(realContent);
		});
	});
	...
	describe("multiline content", () => {
		it("should handle multiline input", async () => {
			...
			const envResult = await env.exec('echo -e "line1\\nline2\\nline3" | tee output.txt');
			const realResult = await runRealBash('echo -e "line1\\nline2\\nline3" | tee output.txt', testDir);
