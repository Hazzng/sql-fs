...
describe("awk - Real Bash Comparison", () => {
	...
	describe("BEGIN and END blocks", () => {
		it("should execute BEGIN before processing", async () => {
			...
			await compareOutputs(env, testDir, "awk 'BEGIN{print \"start\"} {print $0}' data.txt");
		});
		...
		it("should execute END after processing", async () => {
			...
			await compareOutputs(env, testDir, "awk '{print $0} END{print \"done\"}' data.txt");
		});
		...
		it("should execute both BEGIN and END", async () => {
			...
			await compareOutputs(env, testDir, 'awk \'BEGIN{print "start"} {print $0} END{print "end"}\' data.txt');
		});
	});
	...
	describe("stdin input", () => {
		...
		it("should process multi-line piped input", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "printf 'a b\\nc d\\n' | awk '{print $1}'");
		});
	});
