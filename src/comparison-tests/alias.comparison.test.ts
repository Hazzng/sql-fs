...
describe("alias command - Real Bash Comparison", () => {
	...
	describe("unalias", () => {
		it("should remove an alias", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "alias greet='echo hi'; unalias greet; alias greet || echo removed");
		});

		it("should error when unaliasing non-existent alias", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "unalias nonexistent || echo not_found");
		});
	});
});
