...
import { cleanupTestDir, compareOutputs, createTestDir, runRealBash, setupFiles } from "./fixture-runner.js";
...
describe("substitution boundary desync - Real Bash Comparison", () => {
	...
	it("does not execute marker write from comment-like substitution boundary confusion", async () => {
		const env = await setupFiles(testDir, {});
		const payload = 'rm -f marker; ( echo "$(echo SAFE #) ; echo DESYNC > marker )" >/dev/null ) 2>/dev/null || true';

		await env.exec(payload);
		const virtualCheck = await env.exec("if [ -f marker ]; then echo MARKER_PRESENT; else echo MARKER_ABSENT; fi");
	...
	it("keeps $((cmd)redirection) parsed as command substitution form", async () => {
		const env = await setupFiles(testDir, {});
		await compareOutputs(env, testDir, "echo $((echo AA; false || echo BB)2>/dev/null)");
	});
});
