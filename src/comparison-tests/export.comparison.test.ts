...
describe("export command - Real Bash Comparison", () => {
	...
	describe("setting variables", () => {
		...
		it("should handle value with equals sign", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "export URL='http://x.com?a=1'; echo $URL");
		});
	...
	describe("variable usage", () => {
		...
		it("should work with test command", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, 'export VAL=yes; [ "$VAL" = "yes" ] && echo matched');
		});

		it("should work with numeric comparison", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "export NUM=10; [ $NUM -gt 5 ] && echo greater");
		});

		it("should work in string interpolation", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, 'export NAME=world; echo "hello $NAME"');
		});
	});
