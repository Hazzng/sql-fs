...
describe("sed command - Real Bash Comparison", () => {
	...
	describe("stdin", () => {
		it("should read from stdin", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "echo 'hello world' | sed 's/hello/hi/'");
		});

		it("should handle multiline stdin", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "echo -e 'hello\\nworld' | sed 's/o/0/g'");
		});
	});
	...
	describe("multiple expressions", () => {
		it("should apply multiple -e expressions", async () => {
			...
			await compareOutputs(env, testDir, "sed -e 's/hello/hi/' -e 's/world/there/' test.txt");
		});
	});
