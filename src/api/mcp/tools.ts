...
export function registerTools(server: McpServer, sessionManager: SessionManager, owner: string, tenant: string): void {
	server.tool(
		"sandbox_create",
		"Create an isolated bash sandbox with a virtual filesystem. Optional runtime flags opt in to python3/python (CPython WASM, stdlib plus experimental pip-installed pure-Python wheels), js-exec/node (QuickJS WASM), and outbound HTTPS for curl plus git clone/fetch/push. Optional name for human-readable identification.",
		{
	...
	const bashExecDescription = [
		...
		"",
		"NOT supported: apt/npm (no system or JS package managers),",
		...
		"- python3 / python — CPython WASM, stdlib plus installed packages (no network, no os.system).",
		"  Concurrent python3 executions across the server are capped to prevent OOM; excess",
		"  scripts queue until a slot frees.",
		"- pip install / uninstall / list / freeze — experimental, pure-Python wheels only",
		"  (py3-none-any). install needs network:true; the rest do not. Packages land in",
		"  /site-packages, persist with the sandbox, and are reused across sandboxes, so a",
		"  wheel already installed anywhere in the tenant installs with no download.",
	...
	server.tool(
		...
		[
			...
			"",
			"`basePath`: absolute root inside the sandbox (default `/home/user`).",
			"Only files under that prefix are returned, so the default excludes",
			"`/site-packages` — an installed package tree is not exported unless you",
			"pass a broader base path (e.g. `/`), which will include it.",
		].join("\n"),
		{
			id: z.string(),
