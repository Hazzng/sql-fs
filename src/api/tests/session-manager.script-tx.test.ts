...
describe("execWithRuntimeThrottle script-tx wrapping", () => {
	...
	it("Python semaphore still acquired/released around scope", async () => {
		...
		const session = await sm.getOrCreate(T, "wrap-py", { pythonRuntime: "stdlib", javascript: false, network: false });
	...
	it("JS semaphore still acquired/released around scope", async () => {
		...
		const session = await sm.getOrCreate(T, "wrap-js", { pythonRuntime: null, javascript: true, network: false });
