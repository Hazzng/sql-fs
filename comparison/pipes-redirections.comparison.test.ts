...
import { fs, path, cleanupTestDir, compareOutputs, createTestDir, runRealBash, setupFiles } from "./fixture-runner.js";
...
describe("Pipes - Real Bash Comparison", () => {
	...
	describe("multiple pipes", () => {
		...
		it("should chain four commands", async () => {
			...
			// normalizeWhitespace needed because BSD/GNU wc have different column widths
			await compareOutputs(env, testDir, "cat test.txt | grep hello | sort | wc -l", { normalizeWhitespace: true });
		});
		...
		it("should pipe through tr and sort", async () => {
			...
			await compareOutputs(env, testDir, "cat test.txt | tr 'A-Z' 'a-z' | sort");
		});
		...
		it("should pipe through cut and sort", async () => {
			...
			await compareOutputs(env, testDir, "cat test.txt | cut -d: -f2 | sort -n");
		});
	});
	...
	describe("pipes with head and tail", () => {
		...
		it("should pipe head to tail", async () => {
			...
			await compareOutputs(env, testDir, "cat test.txt | head -n 4 | tail -n 2");
		});
	});
});
...
describe("Output Redirections - Real Bash Comparison", () => {
	...
	describe("append redirection (>>)", () => {
		...
		it("should append multiple times", async () => {
			...
			await env.exec('echo "line 2" >> output.txt');
			await runRealBash('echo "line 1" >> output.txt && echo "line 2" >> output.txt', testDir);
...
describe("Command Chaining - Real Bash Comparison", () => {
	...
	describe("&& (AND) chaining", () => {
		it("should execute second command if first succeeds", async () => {
			...
			await compareOutputs(env, testDir, "echo start && cat test.txt && echo end");
		});

		it("should stop on failure", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, 'cat nonexistent.txt 2>/dev/null && echo "never shown"');
		});
	});
	...
	describe("|| (OR) chaining", () => {
		it("should execute second command if first fails", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, 'cat nonexistent.txt 2>/dev/null || echo "file not found"');
		});

		it("should skip second command if first succeeds", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, 'echo "success" || echo "never shown"');
		});
	});
	...
	describe("; (sequential) chaining", () => {
		...
		it("should continue after failure", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, 'cat nonexistent.txt 2>/dev/null; echo "still runs"');
		});
	});
	...
	describe("combined operators", () => {
		it("should handle && and ||", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, 'cat nonexistent.txt 2>/dev/null && echo "yes" || echo "no"');
		});
