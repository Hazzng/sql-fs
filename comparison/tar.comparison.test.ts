...
describe("tar command - Real Bash Comparison", () => {
	...
	describe("create and list (-c -t)", () => {
		it("should create and list single file archive", async () => {
			...
			await compareOutputs(env, testDir, "tar -cf archive.tar hello.txt && tar -tf archive.tar");
		});
		...
		it("should create and list multiple files", async () => {
			...
			await compareOutputs(env, testDir, "tar -cf archive.tar file1.txt file2.txt && tar -tf archive.tar | sort");
		});
		...
		it("should create and list directory archive", async () => {
			...
			await compareOutputs(env, testDir, "tar -cf archive.tar mydir && tar -tf archive.tar | sort");
		});
	});
	...
	describe("gzip compression (-z)", () => {
		it("should create and list gzip compressed archive", async () => {
			...
			await compareOutputs(env, testDir, "tar -czf archive.tar.gz compress.txt && tar -tzf archive.tar.gz");
		});
