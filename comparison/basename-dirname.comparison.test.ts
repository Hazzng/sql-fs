...
describe("basename command - Real Bash Comparison", () => {
	...
	describe("multiple files with -a", () => {
		it("should handle multiple paths with -a", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "basename -a /path/one.txt /path/two.txt");
		});

		it("should handle -a with -s", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "basename -a -s .txt /path/one.txt /path/two.txt");
		});
	});
});
...
describe("dirname command - Real Bash Comparison", () => {
	...
	describe("multiple paths", () => {
		it("should handle multiple paths", async () => {
			const env = await setupFiles(testDir, {});
			await compareOutputs(env, testDir, "dirname /path/to/file1 /another/path/file2");
		});
	});
});
