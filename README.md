...
| `SCRIPT_TX_BUFFER_MAX_OPS` | No | `50000` | Max buffered mutations in one script scope. Past it the script fails with `ENOBUFS` → **413** and **nothing is applied**; the buffer is never flushed early, because that would split one commit into two and break the per-script all-or-nothing guarantee. 5x just-bash's own 10,000-command ceiling, so no single exec can reach it — the heaviest script measured buffered 3,317 operations. |
