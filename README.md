...
| `MAX_FILE_WRITE_BYTES` | No | `52428800` | Largest single file body any write surface accepts (PUT, PATCH, MCP `file_write`), 50 MiB — the contentCache cap. Raising it past that costs ~4x the memory per file for the whole `SESSION_IDLE_MS`. |
| `MAX_BULK_WRITE_BYTES` | No | `MAX_FILE_WRITE_BYTES` | Max total **decoded** bytes in one `POST /writeFiles` batch. Defaults to the per-file cap so the bulk route is not a wider door than the single write it batches (it used to default to 128 MiB — see #168). The request body itself is capped at **twice** this, counted off the stream before the JSON parse, leaving headroom for JSON string escaping. |
| `MAX_EXEC_FILE_BYTES` | No | `8388608` | Largest file a sandbox `exec` script may read whole, or produce with one write (8 MiB). Bash text utilities rebuild strings synchronously on the main thread — `sed s///g` blocks the event loop 706 ms at 8 MiB and 2238 ms at 16 MiB, and a stall past the 2 s Redis `commandTimeout` times out **other** tenants' in-flight commands. Over the cap the script's read/write fails with `EFBIG` → **413**, not retryable. It bounds only calls made from inside `bash.exec`: the HTTP/MCP file routes keep their own caps, so a larger file can still be written with `PUT .../files/{path}` and fetched back with `GET .../files/{path}` — it just cannot be processed in-sandbox. Stopgap until `bash.exec` moves off the main thread (#168). |
| `MAX_BULK_WRITE_FILES` | No | `1000` | Max number of entries in one `POST /writeFiles` batch. |
...
Configure the Redis behind `REDIS_DATA_URL` (which defaults to `REDIS_URL`) with an
`allkeys-*` eviction policy:
...
`volatile-*` is not sufficient. It evicts only keys that carry a TTL, and on the
default single-instance setup the exec-lock leases and version counters carry one
too, so it can reap a live lock lease to make room for a cached blob.

The server checks this at boot with `CONFIG GET maxmemory-policy` and logs
`event:"redis_eviction_policy_unsafe"` at `severity:"critical"` if the policy is not
`allkeys-*`. It is a warning, never a startup failure: managed Redis providers often
