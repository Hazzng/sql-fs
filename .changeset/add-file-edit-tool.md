...
HTTP and MCP share one implementation in `src/api/lib/file-ops.ts` — the edit contract, parent-directory creation and the write cap — so the two surfaces cannot drift on what a write means. The route is documented in the OpenAPI spec.
