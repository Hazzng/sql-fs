...
4. **New integration probe** (acceptance criterion in the issue): 10 readOnly scripts with `sleep 0.5; echo ok` finish in wall-clock < 2 s (was ~5 s). The probe likely belongs in `src/api/tests/integration/` next to the existing cross-replica RW-lock tests.
...
4. **Stress test coverage**: the removed legacy harness exercised `withSessionRead` for cross-request parallelism. A new integration test for "intra-batch parallel readOnly" would mirror the issue's acceptance criterion.
